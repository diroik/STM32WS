...
//    Word   CalcCRC    = TBuffer::Crc(&Buffer->BUF[start], (index-start));
    ...
    { //удалил все управл€ющие символы
      CLEAR_WDT;
...
{
 TBuffer *Buffer     = (TBuffer*)p2;
 Data_Params   BootParams   = *TObject::PtrToConstants; //вз€ли существующие значени€
  ...
  CLEAR_WDT;
      ...
      if( LastDataSize == 3 )
      {
        ...
        BootParams.BOOT_PARAMS.AppAdrStart    = TBuffer::HexToDWord_( (Byte*)TCmd::LastData[0].c_str() );
        BootParams.BOOT_PARAMS.Segment_Count  = TBuffer::HexToByte( (Byte*)TCmd::LastData[1].c_str() );
        BootParams.BOOT_PARAMS.AppCRC         = TBuffer::HexToWord( (Byte*)TCmd::LastData[2].c_str() );
        BootParams.BOOT_PARAMS.AppCurrAdr     = BootParams.BOOT_PARAMS.AppAdrStart;
        if(TObject::Save_ParamsInInfo(&BootParams) == true)
        {
            //GlobalCRC             = 0;
            //GlobalAddr            = 0;
        }
...
TCmd* TCmdUD::CMD_Execute(void* p1,void* p2)
{
 ...
 string PA = "";
        ...
        if( LastDataSize == 2 )
        {
            ...
            Data_Params   BootParams   = *TObject::PtrToConstants;
            BootParams.BOOT_PARAMS.AppCurrAdr       = NewPageAddr;
            TObject::Save_ParamsInInfo(  &BootParams );
...
{ 
       TBuffer *Buffer     = (TBuffer*)p2;
       Word        AppCRC     =  TObject::PtrToConstants->BOOT_PARAMS.AppCRC;
       DWord       StartAddr  =  TObject::PtrToConstants->BOOT_PARAMS.AppAdrStart;
       Word        BlockCNT   =  TObject::PtrToConstants->BOOT_PARAMS.Segment_Count;
       ...
       Word        CalcCRC    =  TBuffer::AppCrc(StartAddr, BlockCNT);
        ...
        {
          Data_Params   BootParams = *TObject::PtrToConstants; //вз€ли существующие значени€
          BootParams.BOOT_PARAMS.BootStatus       = 1;
          BootParams.BOOT_PARAMS.AppCurrAdr       = BootParams.BOOT_PARAMS.AppAdrStart;
          TObject::Save_ParamsInInfo(  &BootParams );
          TCmd::ResetFlg           = true;  
        }
        ...
        { 
          TCmd::LastData.push_back( TBuffer::WordToString(CalcCRC) );
          TCmd::LastData.push_back( TBuffer::WordToString(AppCRC) );
...
TCmd* TCmdGD::CMD_Execute(void* p1,void* p2)
{
 ...
 string Answer;
  ...
  TCmd::LastData.clear();
    ...
    Boot_Params   BootParams   = TObject::PtrToConstants->BOOT_PARAMS; //вз€ли существующие значени€
                  ...
                  TBuffer::WordToHex(BootParams.Segment_Count)+','+
                    TBuffer::WordToHex(BootParams.AppCRC)+','+
                       TBuffer::DWordToHex(BootParams.AppCurrAdr)+','+
                         TBuffer::WordToString(BootParams.BootStatus)+','+
                            TBuffer::WordToString(BootParams.CurVersion);
