...
__task main (void)
{
    /* Set the Vector Table base location at 0x08007400 */ 
  ...
  if( PowerMetrManager != NULL )
  {
      switch( PowerMetrManager->Fase )
      {
        ...
        case FORMING_MESSAGE:
          PowerMetrManager->FASE_FormingMessageCMD( &CMDmessages );
          break;           
        default:
          TObject::Soft_Reset();
      }
...
void InitRS485Objects()
{
    ...
    BaseCmd.AddToCMD_Messages(new TCmd_UnknownCMD(&BaseCmd, "UnknCMD"));  
...
void Init_PowerMetrManagerObjects(void) 
{
  ...
  TPowerMetrManager *Pm = new TPowerMetrManager(ptr1->ReglamentTime);
  Pm->Init(ptr1);
  PowerMetrManager = Pm;
}
//******************************************************************************
