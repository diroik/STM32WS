...
void  TObject::SetDataTime(TDateTime &dt, string &date, string &time)
{
  ...
  if( div > 1)//если разница больша€ - пр€ма€ запись значений времени
  {
  ...
  {
    //flg = true;
    if(NewGlobalSeconds > CurrGloalSeconds)//отстаЄм, нужно быстрее
    { 
      /*if(flg == false)
      {
        RtcTimerDivider -=1;
...
//=============================================================================
// «апись в пам€ть констант  
//=======================================
bool TObject::Save_ParamsInInfo(Data_Params  *Params)
{
//ѕерезапись всей страницы с временным(защитным) хранилищем в другой странице 
 Word         Len;
 Byte        *Tmp                  = (Byte *)Params;  //вз€ли указатель 
  
  Len = sizeof(*Params); 
  for(Word i=0; i<3; i++)
  {
    WriteEepromBufToPage((Byte const*)&TMP_PARAMS, Tmp, Len-1 );  //записали во временную(защитную) пам€ть
...
{
 Word         Len;
 Data_Params  TmpParam            = *PtrToConstants;    //сохранили текущие значени€ параметров в оперативке
 Byte        *Tmp                 = (Byte *)&TmpParam;  //вз€ли указатель 
...
void TTimer::wait_1us(Word cnt)
{
	...
	for (Word i = 0; i < cnt; i++)
        { 
          CLEAR_WDT;
          //__delay_cycles(8);
          //__delay_cycles(8);//Ѕыло 8 но после добавлени€ CLEAR_WDT; 500 мс превратилось в 610...
          //__delay_cycles(9);
        }
        CLEAR_WDT;
}
