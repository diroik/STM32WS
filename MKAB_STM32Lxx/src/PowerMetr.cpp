...
// Constructor
//=======================================
TPowerMetrManager::TPowerMetrManager(Word period):TFastTimer(2,&SecCount),Period(period),Fase(DETECT_STATES), ADCReady(false)
{ 
...
//
//=======================================
void TPowerMetrManager::Init(Data_Params const * DataPtr)
{ 
  ADCReady = false;
 ...
 //Датчики 
  ...
  DATA1->VoltSensors.push_back(new TVoltageDetector(&AKB[4].AKB_OUT, &AKB[0].AKB_OUT, DataPtr->CalVolt[0], 83, Pn,        fullsacle+1,  7.00));//U11
  DATA1->VoltSensors.push_back(new TVoltageDetector(&AKB[3].AKB_OUT, &AKB[0].AKB_OUT, DataPtr->CalVolt[1], 83, Pn,        fullsacle+1, 13.012));//U12
  DATA1->VoltSensors.push_back(new TVoltageDetector(&AKB[2].AKB_OUT, &AKB[0].AKB_OUT, DataPtr->CalVolt[2], 83, Pn,        fullsacle+1, 20.50));//U13
  DATA1->VoltSensors.push_back(new TVoltageDetector(&AKB[1].AKB_OUT, &AKB[0].AKB_OUT, DataPtr->CalVolt[3], 83, Pn,        fullsacle+1, 25.00));//U14
  
  DATA1->TempSensors.push_back(new TVoltageDetector(&AKB[8].AKB_OUT, &AKB[0].AKB_OUT, DataPtr->CalVolt[4], 83, (Pn+1),   fullsacle+1,  7.00));//U21
  DATA1->TempSensors.push_back(new TVoltageDetector(&AKB[7].AKB_OUT, &AKB[0].AKB_OUT, DataPtr->CalVolt[5], 83, (Pn+1),   fullsacle+1, 13.012));//U22
  DATA1->TempSensors.push_back(new TVoltageDetector(&AKB[6].AKB_OUT, &AKB[0].AKB_OUT, DataPtr->CalVolt[6], 83, (Pn+1),   fullsacle+1, 20.50));//U23
  DATA1->TempSensors.push_back(new TVoltageDetector(&AKB[5].AKB_OUT, &AKB[0].AKB_OUT, DataPtr->CalVolt[7], 83, (Pn+1),   fullsacle+1, 25.00));//U24
         ...
         Version+="\r\n>,100,"+TBuffer::WordToString(addr)+","+
                          TBuffer::WordToString(DEVICE_CURR_VER)+","+
                            TBuffer::WordToString(DEVICE_SUB_VER)+","+
                              TBuffer::WordToString(ptr->BOOT_PARAMS.CurVersion)+",";
