...
//Перезапись всей страницы с временным(защитным) хранилищем в другой странице 
 Word         Len;
 Byte        *Tmp                  = (Byte *)BootParams;  //взяли указатель 
  
  Len = sizeof(*BootParams); 
  for(Word i=0; i<3; i++)
  {
    WriteBufToPage((Byte const*)&TMP_BOOT_PARAMS, Tmp, Len-1 );  //записали во временную(защитную) память
    WriteBufToPage((Byte const*)&BOOT_PARAMS    , Tmp, Len-1 );  //записали куда надо
...
{
 Word         Len;
 Boot_Params  TmpParam            = *PtrToBootConstants;    //сохранили текущие значения параметров в оперативке
 Byte        *Tmp                 = (Byte *)&TmpParam;  //взяли указатель 
...
void TObject::RestoreBootDefault(void)
{
 ...
 Byte        *Tmp;
        ...
        CLEAR_WDT;
        Tmp         = (Byte *)&TmpBootParam;  //взяли указатель 
...
//=============================================================================
//нельзя записывать меньше чем 4 байта
//=======================================
  ...
  __disable_interrupt(); 
      switch (port)
      {
        case COM0:
          ...
          // 38400 Делитель = 16000000/(16*38400) = 26.0416
          USART1->BRR =(13<<4); 	// Целая часть 26
          //USART2->BRR |= 13; 	    // Дробная часть коэффициента деления USART 0.041 * 16 = 0.6
