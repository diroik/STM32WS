...
#define I_TRANSFER_TO_STORAGE 5   // ток перехода в STORAGE mA
#define U_STORAGE_HOLD 6.5        // напряжение поддержания в состоянии STORAGE
#define I_LOAD_MAX 2000           // максимальный ток нагрузки
...
//******************************************************************************  
        ...
        int dac_step=DAC_STEP; // шаг DAC в зависимости от условий
...
//float MAX_Iload=0;// = PowerMetrManager->PowerMetrs->CurrSensors[1];    
...
__enable_interrupt();
  ...
  {    
    ...
    { 
        ...
        //*******загрузка значения в dac**********************//
       if (dac<=0)           // ограничиваем dac от перескока 0 и по максимальному значению
            dac=0;
        else if (dac>=4096)
                 dac=4096;      
        DAC->DHR12RD=dac; 
...
void Indicate_Process(void)
{
  ...
  if (AppPowerState==STATE_STORAGE)
  {
    if (Batt<U_STORAGE_HOLD-DELTA_U_MAX)
       Indicate_Set(LED_LOAD_CHARGE_HOLD, COLOR_RED);
    else if (Batt>U_STORAGE_HOLD-DELTA_U_MAX)
            Indicate_Set(LED_LOAD_CHARGE_HOLD, COLOR_GREEN); 
  }
}
...
void Do_STATE_PRECHARGE(void)
{
  ...
  if (Uin<U_IN_CONST)  AppPowerState=STATE_DISCHARGE;//переход в разряд
}
...
void Do_STATE_CHARGE(void)//V
{
  ...
  if((Icharge<I_CHARGE_MAX) && (Batt<U_BAT_MAX))
  { 
     if (dac_step>DAC_STEP_MIN) 
       dac_step--;     
     if (dac_step<DAC_STEP_MIN) 
       dac_step=DAC_STEP_MIN; 
     dac+=dac_step;       // прирост DAC и постепенное глушение шага
  ...
  else if ((Icharge>I_CHARGE_MAX) && (Batt<U_BAT_MAX))
  {
           ...
           if (dac_step<DAC_STEP_MIN)
             dac_step=DAC_STEP_MIN;          
           dac-=dac_step;       // глушим DAC  и постепенное глушение шага          
  }
  if(Batt>U_BAT_MAX) 
...
void Do_STATE_FULL_CHARGE(void)
{
  ...
  if (Uin<U_IN_CONST)  AppPowerState=STATE_DISCHARGE;//переход в разряд
  if ((Icharge-Iload)<=I_TRANSFER_TO_STORAGE) AppPowerState=STATE_STORAGE;
}
...
void Do_STATE_DISCHARGE(void)//V
{
  ...
  if(Uin>U_IN_CONST) AppPowerState=STATE_PRECHARGE;//переход в разряд
...
void Do_STATE_STORAGE(void)
{
   ...
   if((Ibat<=0) && (Batt<U_STORAGE_HOLD))
     dac+=DAC_STEP;    // прирост DAC
   if ((Ibat>=0) && (Batt>U_STORAGE_HOLD)) 
     dac-=DAC_STEP;    // глушим DAC
   
   //  if((Batt>(U_STORAGE_HOLD+DELTA_U_MAX)) && ((Ibat<=I_TRANSFER_TO_STORAGE) && (abs(Ibat)<6))) 
   //     dac-=DAC_STEP;    // глушим DAC
   //  else if (Batt<(U_STORAGE_HOLD-DELTA_U_MAX)) 
   //       dac+=DAC_STEP;    // прирост DAC  
}
